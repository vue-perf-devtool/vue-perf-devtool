...
function initApp (shell) {
  shell.connect(bridge => {
    ...
    bridge.once('ready', version => {
      ...
      bridge.send('perf:toggle-recording', store.state.perf.enabled)
    ...
    bridge.on('flush', payload => {
      store.commit('perf/FLUSH', parse(payload))
    })

    bridge.on('perf:measure', payload => {
      store.commit('perf/RECEIVE_MEASURE', parse(payload))
      if (store.state.tab !== 'events') {
        store.commit('perf/INCREASE_NEW_EVENT_COUNT')
      }
    })
